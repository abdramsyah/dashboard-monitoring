...
import (
	...
	auth "sentadel-backend/internal/auth"
	authMock "sentadel-backend/internal/auth/mock"
	cryptoMock "sentadel-backend/internal/base/crypto/mock"
	baseErrors "sentadel-backend/internal/base/errors"
	user "sentadel-backend/internal/user"
	userMock "sentadel-backend/internal/user/mock"
)
...
func TestAuthUsecases_Login(t *testing.T) {
	...
	t.Run("expect it logins user", func(t *testing.T) {
		...
		prep.userRepo.EXPECT().GetByEmail(mock.Anything, in.Username).Return(getUser, nil)
	...
	t.Run("expect it fails if user with such email does't exist", func(t *testing.T) {
		...
		prep.userRepo.EXPECT().GetByEmail(mock.Anything, in.Username).Return(getUser, err)
